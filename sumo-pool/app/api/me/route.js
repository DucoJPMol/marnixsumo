...
import { performAction, GameError } from "../../../lib/game";
...
// A POST so the login token never lands in a URL or a CDN cache.
export async function POST(request) {
  ...
  try {
    const result = await performAction(store, { type: "session", ...body });
    ...
    return NextResponse.json({ error: "Er ging iets mis op de server.", code: "server_error" }, { status: 500, headers: NO_STORE });
  }
}
