...
async function placeBet(store, body) {
  ...
  const amt = Math.round((Number(body.amt) || 0) * 100) / 100;
  const pick = String(body.pick || "");
  if (!Number.isFinite(amt) || amt < 0) throw new GameError("Vul een geldig aantal MX in.");
  ...
  return withLock(store, K.userLock(uid), async () => {
    ...
    if (amt < MIN_BET - EPS) throw new GameError(`De minimale inzet is ${mx(MIN_BET)}.`);
    const available = finance.balance + (existing ? existing.amt : 0);
    if (amt > available + EPS) throw new GameError(`Je hebt nog ${mx(Math.max(0, available))}.`, 400, "funds");
